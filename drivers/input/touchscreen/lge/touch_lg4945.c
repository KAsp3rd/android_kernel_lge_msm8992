...
int lg4945_ic_info(struct device *dev)
{
	...
	ts->xfer->data[4].rx.addr = 0x8A46;
	ts->xfer->data[4].rx.buf = (u8 *)&version_hidden;
	ts->xfer->data[4].rx.size = sizeof(version_hidden);
...
static int lg4945_notify(struct device *dev, ulong event, void *data)
{
	...
	case NOTIFY_CALL_STATE:
		...
		ret = lg4945_reg_write(dev, REG_CALL_STATE,
			(u32*)data, sizeof(u32));
		break;
	default:
...
{
	INIT_DELAYED_WORK(&d->font_download_work, ext_watch_font_download_func);
}
...
static int lg4945_upgrade(struct device *dev)
{
	...
	int ret = 0;
	int i = 0;
	int display_dic_rev = lge_get_lgd_sic4945_rev();
	...
	if (ts->test_fwpath[0]) {
		...
		TOUCH_I("fw_rev = %d, dic_rev = %d\n", d->fw.revision, display_dic_rev);
		if (d->fw.revision == display_dic_rev) {
			if (d->fw.revision != 3)
			...
			if (display_dic_rev != 3)
				memcpy(fwpath, ts->def_fwpath[1], sizeof(fwpath));
			else
				memcpy(fwpath, ts->def_fwpath[0], sizeof(fwpath));
			TOUCH_I("fw_rev != dic_rev - get fwpath from def_fwpath : %s\n", fwpath);
		}
	} else {
...
static int lg4945_init(struct device *dev)
{
	...
	ret = lg4945_reg_write(dev, REG_CALL_STATE, &data, sizeof(u32));
	if (ret)
		TOUCH_E("failed to write \'REG_CALL_STATE\', ret:%d\n", ret);
	...
	if (ret)
		TOUCH_E("failed to init watch cfg, ret:%d", ret);

	ret = ext_watch_check_font_download(d->dev);
	if (ret)
		TOUCH_I("skip font download, ret:%d", ret);
...
static int lg4945_irq_handler(struct device *dev)
{
	...
	if (report_mode) {
		...
		sic_abt_report_mode(dev, &all_data[4]);
	} else {
		if (d->info.wakeup_type == ABS_MODE) {
			if (sic_abt_is_debug_mode()) {
				sic_abt_onchip_debug(dev, &all_data[4]);
				ret = lg4945_irq_abs(dev);
			} else {
				sic_abt_ocd_off(dev);
				ret = lg4945_irq_abs(dev);
...
static int lg4945_register_sysfs(struct device *dev)
{
	...
	watch_register_sysfs(dev);
	prd_register_sysfs(dev);
	sic_abt_register_sysfs(&ts->kobj);
