...
#ifndef CONFIG_LGE_PM_RTC_PWROFF_ALARM
static unsigned long rtc_fake_secs;
#else
unsigned long rtc_fake_secs;
#endif
#endif
...
extern unsigned long misc_pwron_secs;
#else
...
#ifdef CONFIG_LGE_PM_RTC_PWROFF_ALARM
	pr_debug("[%s]: enabled : %d, poweron_alarm : %d\n",
...
#ifdef CONFIG_LGE_RTC_FAKE_SECS
	rtc_fake_secs = mktime(2014, 1, 1, 0, 0, 0);
#endif
...
static void qpnp_rtc_shutdown(struct spmi_device *spmi)
{
	...
	if (rc)
		dev_err(rtc_dd->rtc_dev, "SPMI write failed\n");
		goto rtc_rw_fail;
	} else if(poweron_alarm) {

	/* When power off alarm enable */
	...
	pr_debug("[%s %d] secs = %lu, misc_secs = %lu\n", __func__, __LINE__, secs, misc_pwron_secs);

	if(misc_pwron_secs != secs)
		secs = misc_pwron_secs;
	...
	rc = qpnp_write_wrapper(rtc_dd, value,
			...
			NUM_8_BIT_RTC_REGS);
	if (rc) {
		dev_err(rtc_dd->rtc_dev, "Write to ALARM reg failed\n");
		goto rtc_rw_fail;
	}
	...
	rc = qpnp_write_wrapper(rtc_dd, &ctrl_reg,
			rtc_dd->alarm_base + REG_OFFSET_ALARM_CTRL1, 1);
	if (rc) {
		dev_err(rtc_dd->rtc_dev, "Write to ALARM cntrol reg failed\n");
		goto rtc_rw_fail;
	}
	rc = qpnp_read_wrapper(rtc_dd, &ctrl_reg,
			rtc_dd->alarm_base + REG_OFFSET_ALARM_CTRL1, 1);
