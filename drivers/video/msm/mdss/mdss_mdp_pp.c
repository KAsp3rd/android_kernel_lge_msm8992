...
int pp_set_dmb_status(int flag) {
	dmb_status = flag;
	pr_info("%s : dmb_status=%d from %pS\n", __func__, dmb_status,
...
static int prev_cam_preview_tune_status = -1;
#define IS_CAM_PREVIEW_TUNE_STATUS_CHANGED(s) do { \
	if (s != prev_cam_preview_tune_status) {\
		pr_info("tune status changed to %d\n", s);\
		prev_cam_preview_tune_status = s;\
	}\
} while(0)

int pp_set_cam_preview_tune_status(int flag) {
	cam_preview_tune_status = flag;
	pr_info("%s : cam_preview_tune_status=%d from %pS\n", __func__, cam_preview_tune_status,
		__builtin_return_address(0));
	return 0;
}
...
extern int mdss_dsi_panel_ad_apply(unsigned int enabled);
#endif
...
#else
			IS_CAM_PREVIEW_TUNE_STATUS_CHANGED(cam_preview_tune_status);
...
#if defined(CONFIG_LGE_PP_AD_SUPPORTED)
	// mdss_dsi_panel_ad_apply(ad_support);
#endif
ad_config_exit:
	mutex_unlock(&ad->lock);
	return ret;
